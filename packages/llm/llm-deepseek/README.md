...
- Tool-call deltas require a nonnegative safe-integer `index` and string-or-null argument fragments. Deltas sharing an index assemble into one call; empty or null continuation `id` and `name` fields do not erase established identity, while a completed call that never supplies a non-empty id and name fails as `MALFORMED_RESPONSE` instead of becoming an executable empty-identity call.
- Required prompt/completion usage counts and reported cache/reasoning counts must be nonnegative safe integers. Null optional counts are omitted, and cache reads greater than total prompt tokens fail as `MALFORMED_RESPONSE`.
